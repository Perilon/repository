<?php
/* Msgs Class - coursesweb.net/php-mysql/ */
class Msgs extends Base {
  public $table = 'msgs';           // HERE add the mysql table name
  ...
  protected function setMsgs() {
    if($list = $this->getMysqlRows('*', $this->table, "WHERE `user`='$this->user' AND LENGTH(`msg`)>4", 'ORDER BY `id` ASC')) {
      ...
      for($i=0; $i<$this->affected_rows; $i++) {
        // name of user which added msg, set 'title' attribute with Facebook_id, 'yahoo' or 'google' string (used in 'msgs.js - showImgBox()' to create link), if 'fbuserid', or 'social' columns not '0'
        $name = $list[$i]['name'];
        $nametitle_id = '';
        ...
        // If Admin, or its the page of the logged user ($_SESSION['usritspage'] is 1, defined in "users"), add Delete
