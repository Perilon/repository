...
class Users extends Base {
  ...
  public function __construct($conn_data) {
    ...
    if(!isset($_POST['isajax'])) {
      $this->loged = '<form action="" method="post" id="log_form">'.$this->lsite['users_logform']['email'].': <input type="text" name="email" id="email" size="12" maxlength="55" /> '.$this->lsite['users_logform']['pass'].'<input type="password" name="pass" id="pass" size="12" maxlength="18" /><input type="submit" name="login"  class="submit" value="'.$this->lsite['users_logform']['login'].'" /><br/><label for="rem" id="lrem"><input type="checkbox" name="rem" id="rem" />'.$this->lsite['users_logform']['rem'].'</label><hr/><a href="'. TOUSRF. '?rc=Recover" title="'.$this->lsite['users_logform']['recdat'].'" id="recdat">'.$this->lsite['users_logform']['recdat'].'</a> <a href="'. TOUSRF. '?susr=Register" title="'.$this->lsite['users_logform']['register'].'" id="linkreg">'.$this->lsite['users_logform']['register'].'</a>';
  ...
  private function setLoged() {
    // if user loged with Facebook, or else with Google
  ...
  private function getLogin($ar_post) {
    ...
    if(!preg_match("/^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,3})$/", strtolower($ar_post['email'])) || !$this->checkStr($ar_post['pass'])) $this->eror = $this->lsite['eror_email'];
  ...
  private function confirmUser($email, $passenc) {
    ...
    $sql = "SELECT `id`, `name`, `passenc`, `rank`, `dtvisit`, `language` FROM `users` WHERE `email`='$email' LIMIT 1";
    ...
    if(!$row || $this->affected_rows<1) return 1;         // Indicates email not registered
      ...
      if($row[0]['rank'] < 0) return 4;               // User banned
		...
		$_SESSION['language'] = $row[0]['language'];
  ...
  private function usersOn($sl=0) {
    ...
    // If the user is logged in, but not with Facebook/Yahoo/Google, insert or update him in 'useron'
